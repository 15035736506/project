...
BOOL CUDPcarDlg::InitSocket()
{
	...
	//建立基于TCP的套接字
/*	m_socketcmd=socket(AF_INET,SOCK_STREAM,0);//TCP
	if(INVALID_SOCKET==m_socket)
	{
		MessageBox("套接字创建失败TCP！");
		return FALSE;
	}
	SOCKADDR_IN addrSockcmd;
	addrSockcmd.sin_family=AF_INET;
	addrSockcmd.sin_port=htons(1111);
	addrSockcmd.sin_addr.S_un.S_addr=htonl(INADDR_ANY);

	int retvalcmd;
	retvalcmd=bind(m_socketcmd,(SOCKADDR*)&addrSock,sizeof(SOCKADDR));
	if(SOCKET_ERROR==retval)
	{
		closesocket(m_socket);
		MessageBox("绑定失败TCP");
...
BOOL CUDPcarDlg::PreTranslateMessage(MSG* pMsg) 
{
	...
	static CString strSend;//待发送的命令
	static CString strSendTmp;//待发送的命令
	...
	while(1)
	{
		...
		else
		{
			for(i=0;i<160;i++)
			{
				...
				picDraw.SetPixel(i+160,j,RGB(R,G,B));
			}
		}
