...
//  	FRAMEWIN_SetDefaultFont(&GUI_FontHZ_MicrosoftFont_13);
	FRAMEWIN_SetDefaultFont(&GUI_Font13_ASCII);
...
//	BUTTON_SetDefaultFont(&GUI_FontHZ_MicrosoftFont_13);
	...
	/**************************************************************/
	 /* Create the button*/   
    hButton_mainmenu[0] = BUTTON_Create(5, 25, 90, 90, GUI_ID_BUTTON0, WM_CF_SHOW);   
	hButton_mainmenu[1] = BUTTON_Create(100, 25, 90, 90, GUI_ID_BUTTON1, WM_CF_SHOW);
	hButton_mainmenu[2] = BUTTON_Create(195, 25, 90, 90, GUI_ID_BUTTON2, WM_CF_SHOW);   

	hButton_mainmenu[3] = BUTTON_Create(5, 120, 90, 90, GUI_ID_BUTTON3, WM_CF_SHOW);
	hButton_mainmenu[4] = BUTTON_Create(100, 120, 90, 90, GUI_ID_BUTTON4, WM_CF_SHOW);
    hButton_mainmenu[5] = BUTTON_Create(195, 120, 90, 90, GUI_ID_BUTTON5, WM_CF_SHOW); 

	hButton_mainmenu[6] = BUTTON_Create(5, 215, 90, 90, GUI_ID_BUTTON6, WM_CF_SHOW); 
	hButton_mainmenu[7] = BUTTON_Create(100, 215, 185, 90, GUI_ID_BUTTON7, WM_CF_SHOW); 
  ...
  /* Set the button text */   
	BUTTON_SetText(hButton_mainmenu[0]," music");//	音乐
	BUTTON_SetText(hButton_mainmenu[1]," memory"); //	存储设备
	BUTTON_SetText(hButton_mainmenu[2]," text");  //  文本
	BUTTON_SetText(hButton_mainmenu[3]," settings"); //	设置
	BUTTON_SetText(hButton_mainmenu[4]," picture");  // 图片
	BUTTON_SetText(hButton_mainmenu[5]," FFT");	  //
	BUTTON_SetText(hButton_mainmenu[6]," USART"); //串口应用
	BUTTON_SetText(hButton_mainmenu[7]," wave");//示波器
...
/*	BUTTON_SetBitmapEx (hButton_mainmenu[0],0, &bmmusic4pp, 25, 15);
	...
	while(WM_GetWindowOrgY(hButton_mainmenu[0]) > 25) //按钮1的左上角，y坐标
	{
...
** input parameters:   s16 x,s16 y,：最终达到的坐标（对话框左上角的坐标）
...
void WM_MoveCtrl(void)
{
  ...
  GUI_TOUCH_GetState(&TouchPoint[1]);
	...
	if (TouchFlag == TouchUnPress) {
	  ...
	  if(WM_GetWindowSizeY(Menu_Info.EndHandle) + WM_GetWindowOrgY(Menu_Info.EndHandle) - WM_GetWindowOrgY(Menu_Info.FirstHandle) < 270){
		} else {
		  i = 240 - WM_GetWindowSizeY(Menu_Info.EndHandle) - 5;
	    /* 实现滑动到最底部向上的回弹效果 */	
